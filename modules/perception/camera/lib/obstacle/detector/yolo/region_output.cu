...
__global__ void get_object_kernel(int n,
  ...
  for (int i = blockIdx.x * blockDim.x + threadIdx.x; i < (n);
    ...
    if (with_box3d) {
      ...
      int offset_dim = loc_index * num_classes * 3 + max_index * 3;
...
void get_objects_gpu(const YoloBlobs &yolo_blobs,
  ...
  int num_anchor_per_scale = num_anchor / 3;
  ...
  const float* loc_data_vec[3] = {yolo_blobs.det1_loc_blob->gpu_data(),
                                  yolo_blobs.det2_loc_blob->gpu_data(),
                                  yolo_blobs.det3_loc_blob->gpu_data()};
  const float* obj_data_vec[3] = {yolo_blobs.det1_obj_blob->gpu_data(),
                                  yolo_blobs.det2_obj_blob->gpu_data(),
                                  yolo_blobs.det3_obj_blob->gpu_data()};
  const float* cls_data_vec[3] = {yolo_blobs.det1_cls_blob->gpu_data(),
                                  yolo_blobs.det2_cls_blob->gpu_data(),
                                  yolo_blobs.det3_cls_blob->gpu_data()};
  const float* ori_data_vec[3] = {get_gpu_data(model_param.with_box3d(),
                                    *yolo_blobs.det1_ori_blob),
                                  get_gpu_data(model_param.with_box3d(),
                                    *yolo_blobs.det2_ori_blob),
                                  get_gpu_data(model_param.with_box3d(),
                                    *yolo_blobs.det3_ori_blob)};
  const float* dim_data_vec[3] = {get_gpu_data(model_param.with_box3d(),
                                    *yolo_blobs.det1_dim_blob),
                                  get_gpu_data(model_param.with_box3d(),
                                    *yolo_blobs.det2_dim_blob),
                                  get_gpu_data(model_param.with_box3d(),
                                    *yolo_blobs.det3_dim_blob)};
  ...
  int all_scales_num_candidates = num_candidates_vec[0] +
                                  num_candidates_vec[1] +
                                  num_candidates_vec[2];
  ...
  //for (int ij=0; ij<all_scales_num_candidates; ij++){
  //  AINFO << "res_cls_data: " << ij;
  //  AINFO << cpu_cls_data[num_classes * all_scales_num_candidates+ij];
  //}
  //const float *cpu_box_data = yolo_blobs.res_box_blob->cpu_data();
  //for (int ij=0; ij<kBoxBlockSize; ij++){
  //  AINFO << "res_box_data: " << ij;
  //  AINFO << cpu_box_data[(all_scales_num_candidates-1)*kBoxBlockSize + ij];
  //}
